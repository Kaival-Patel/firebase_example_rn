...
export const updateChatRoom = async ({chatRoomId,chatRoomModel}) => {
  return await firestore().collection('chat_rooms').doc(chatRoomId).update(chatRoomModel);
};

export const sendMessageInChatRoom = async ({chatRoomId,message}) => {
  return await firestore().collection('chat_rooms').doc(chatRoomId).collection("messages").add(message);
};
